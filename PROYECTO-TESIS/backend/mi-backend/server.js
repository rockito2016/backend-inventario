...
//   const { fecha, detalle_id, ingresos, gastos } = req.body;

//   // Validar que todos los campos necesarios estén presentes
//   if (!fecha || !detalle_id || (!ingresos && !gastos)) {
...
//     fecha,
//     detalle_id,
//     ingresos || null, // Si ingresos no se envía, se establece como NULL
//     gastos || null // Si gastos no se envía, se establece como NULL
//   ];
...
app.post("/balance-agregar", (req, res) => {
  ...
  if (!fecha && !detalle_id && ingresos === null && gastos === null) {
    return res.status(400).json({ error: "Se requiere al menos un dato para registrar." });
  }
...
app.get("/api/tipos-detalle", (req, res) => {
  ...
  connection.query(query, (error, results) => {
    if (error) {
      ...
      return res.status(500).json({ error: "Error al obtener los tipos de detalle" });
    }
...
app.get("/inventario/productos-vencidos", (req, res) => {
  ...
  connection.query(query, (error, results) => {
    if (error) {
      ...
      return res.status(500).json({ error: 'Error en la consulta de productos vencidos' });
    }
    res.json(results);
  });
});
...
app.get("/inventario/productos-proximos-vencer", (req, res) => {
  ...
  connection.query(query, (error, results) => {
    if (error) {
      ...
      return res.status(500).json({ error: 'Error en la consulta de productos próximos a vencer' });
    }
    res.json(results);
  });
});
...
app.put("/inventario/elaborar-agregar", (req, res) => {
  ...
  connection.beginTransaction(error => {
    ...
    connection.query(queryIncrementar, [cantidadDestino, nombre, elaboracionDestino], (error, result) => {
      if (error) {
        return connection.rollback(() => {
          console.error('Error al incrementar cantidad en el producto destino:', error);
          res.status(500).json({ error: 'Error al incrementar la cantidad en el destino' });
        });
      }
      ...
      connection.query(queryRestarUno, [nombre], (error, result) => {
        if (error) {
          return connection.rollback(() => {
            console.error('Error al restar 1 unidad en el producto de origen:', error);
            res.status(500).json({ error: 'Error al restar 1 unidad en el origen' });
          });
        }
        ...
        connection.commit(error => {
          ...
          res.status(200).json({ message: 'Elaboración completada exitosamente' });
        });
      });
    });
  });
...
app.post("/pedido/id-producto-manual", (req, res) => {
  ...
  const sql = "UPDATE producto SET estado_id = 2 WHERE codigo IN (?) AND estado_id = 1";
...
app.post("/pedido/envios-agregar", (req, res) => {
  ...
  if (!nuevoEnvio.transportador_id || !nuevoEnvio.kilos || !nuevoEnvio.precio_kilo || !nuevoEnvio.total_cajas) {
    console.error("Datos incompletos para agregar un envío:", nuevoEnvio);
    return res.status(400).json({ error: "Todos los campos son obligatorios (transportador_id, kilos, precio_kilo, total_cajas)." });
  }
...
app.post("/compras/compra-completa", (req, res) => {
  const { envio_id, transportador_id, kilos, precio_kilo, total_cajas, proveedores } = req.body;
...
function procesarProveedores(envio_id, proveedores, res) {
  const proveedorPromises = proveedores.map((proveedor) => {
    return new Promise((resolve, reject) => {
      ...
      connection.query(queryCompraGeneral, compraGeneralValues, (error, compraResult) => {
        if (error) {
        ...
        const detallePromises = proveedor.productos.map((producto) => {
          return new Promise((resolve, reject) => {
            connection.query(
              ...
              (error) => {
                if (error) {
                  console.error("Error al insertar en compra_detalle:", error);
                  return reject(error);
                }
                resolve();
              }
  ...
  Promise.all(proveedorPromises)
    ...
    .catch((error) => {
      ...
      res.status(500).json({ error: "Error al registrar las compras completas" });
    });
}

// Ruta para agregar un crédito de compra
app.post("/compras/creditos/agregar", (req, res) => {
  const { compra_general_id, fecha, proveedor_id, abono_inicial, abono, total_a_pagar } = req.body;
  ...
  const values = [compra_general_id, fecha, proveedor_id, abono_inicial, abono, total_a_pagar];

  connection.query(queryCreditosCompras, values, (error, result) => {
    if (error) {
      console.error("Error al insertar en creditos_compras:", error);
      return res.status(500).json({ error: "Error al registrar el crédito de compra" });
    }
...
app.post("/ventas/venta-completa", (req, res) => {
  const { fecha, cliente_id, forma_pago_id, abono_inicial, total_a_pagar, productos } = req.body;
  ...
  connection.query(queryVentaGeneral, [fecha, cliente_id, forma_pago_id], (error, results) => {
    if (error) {
      console.error("Error al insertar en venta_general:", error);
      return res.status(500).json({ error: "Error al registrar venta general" });
    }
    ...
    if (forma_pago_id === 2) {
      ...
      const valuesCreditosVentas = [ventaId, fecha, cliente_id, abono_inicial || 0, 0, total_a_pagar];

      connection.query(queryCreditosVentas, valuesCreditosVentas, (error) => {
        if (error) {
          console.error("Error al insertar en creditos_ventas:", error);
          return res.status(500).json({ error: "Error al registrar crédito" });
        }
      });
    }
      ...
      return new Promise((resolve, reject) => {
        ...
        connection.query(
          ...
          (error) => {
            ...
            connection.query(queryUpdateInventario, [producto.cantidad, producto.codigo], (error) => {
              if (error) {
                console.error("Error al actualizar el inventario de producto:", error);
                return reject(error);
              }
              resolve();
            });
    ...
    Promise.all(detallePromises)
      .then(() => {
        res.status(201).json({ message: "Venta completa insertada exitosamente y el inventario actualizado" });
      })
      .catch((error) => {
        console.error("Error en la inserción de detalles de venta o actualización de inventario:", error);
        res.status(500).json({ error: "Error al insertar detalles de venta o actualizar inventario" });
      });
  });
});
...
app.get("/contactos/cliente", (req, res) => {
  const query = "SELECT id, nombre, cedula, celular, correo, direccion FROM cliente";
  connection.query(query, (error, results) => {
    if (error) {
      return res.status(500).json({ error });
    }
...
/* Agregar un crédito de compra */
// app.post("/creditos/compras-agregar", (req, res) => {
//   const nuevoCredito = req.body;

//   console.log("Datos recibidos para agregar crédito de compra:", nuevoCredito); // Verifica los datos recibidos
...
//   const values = [
//     nuevoCredito.compra_general_id,
//     nuevoCredito.fecha,
//     nuevoCredito.proveedor_id,
//     nuevoCredito.abono_inicial,
//     nuevoCredito.abono,
//     nuevoCredito.total_a_pagar
//   ];
...
app.get("/api/formas-pago", (req, res) => {
  ...
  connection.query(query, values, (error, results) => {
    if (error) {
      ...
      return res.status(500).json({ error: "Error al obtener las formas de pago" });
    }
...
app.get("/api/transportadores", (req, res) => {
  ...
  connection.query(query, values, (error, results) => {
    if (error) {
      ...
      return res.status(500).json({ error: "Error al obtener los transportadores" });
    }
